...
export const clientMapper = {
  ...
  filtersToDto(filters: ClientFilters, userId: string): ClientListRequestDto {
    const dto: Omit<ClientListRequestDto, 'identificacion' | 'nombre'> & {
      identificacion?: string;
      nombre?: string;
    } = { usuarioId: userId };
    if (filters.identification !== undefined && filters.identification !== '') {
      dto.identificacion = filters.identification;
    }
  ...
  draftToCreateDto(draft: ClientDraft, userId: string): ClientCreateDto {
    return {
      ...
      telefonoCelular: draft.mobilePhone,
      ...
      sexo: draft.gender,
      resenaPersonal: draft.personalReview,
      imagen: draft.image ?? '',
